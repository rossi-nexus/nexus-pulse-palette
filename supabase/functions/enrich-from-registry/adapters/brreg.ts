...
export const brregAdapter: RegistryAdapter = {
  id: "brreg",
  name: "BRREG (Norway)",
  ...
  validateOrgNumber(raw: string) {
    ...
    if (digits.length !== 9) {
      return { ok: false, error: "Norwegian org numbers must be exactly 9 digits." };
    }
  ...
  async lookupByOrgNumber(orgNumber: string): Promise<Response> {
    ...
    try {
      ...
      return jsonResponse({ error: "Unexpected response shape from BRREG." }, 502);
    }
    ...
    return jsonResponse({
      mode: "single",
      proposal,
      source: { registry: "BRREG", source_url: url },
    });
  },
