...
serve(async (req) => {
  ...
  try {
    ...
    // Validate
    if (typeof extracted_text !== "string" || !extracted_text.trim()) {
      return new Response(
        JSON.stringify({ error: "No readable content in document" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (typeof source_description !== "string" || !source_description.trim()) {
      return new Response(
        JSON.stringify({ error: "Missing source_description" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const truncatedText =
      extracted_text.length > MAX_SOURCE_CHARS
        ? extracted_text.slice(0, MAX_SOURCE_CHARS) + "\n\n[Content truncated]"
        : extracted_text;
