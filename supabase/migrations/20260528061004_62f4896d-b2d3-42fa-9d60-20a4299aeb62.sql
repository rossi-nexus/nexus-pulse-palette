-- Drop the legacy 4-arg overload (partial-write code path).
DROP FUNCTION IF EXISTS public.fn_onboard_verified_actor(jsonb, jsonb, jsonb, uuid);

-- Replace the 5-arg overload: switch the two ontology-tag INSERTs from
-- hardcoded source='manual' to COALESCE(v_*->>'source', 'consultant_completion').
