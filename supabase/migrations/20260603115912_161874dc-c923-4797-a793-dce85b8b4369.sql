...
-- AX4: Outcome capture loop — interactions table, user preferences, v2 RPC with engagement axis.

-- 1) user_actor_interactions
CREATE TABLE public.user_actor_interactions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  actor_id uuid NOT NULL REFERENCES public.actors(id) ON DELETE CASCADE,
  session_id uuid REFERENCES public.search_sessions(id) ON DELETE SET NULL,
  interaction_type text NOT NULL CHECK (interaction_type IN (
    'result_viewed', 'profile_opened', 'included',
    'saved_for_later', 'compared', 'unincluded'
  )),
  metadata jsonb,
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX idx_uai_user_actor ON public.user_actor_interactions(user_id, actor_id);
CREATE INDEX idx_uai_actor_type ON public.user_actor_interactions(actor_id, interaction_type);
CREATE INDEX idx_uai_session ON public.user_actor_interactions(session_id) WHERE session_id IS NOT NULL;

GRANT SELECT, INSERT ON public.user_actor_interactions TO authenticated;
GRANT ALL ON public.user_actor_interactions TO service_role;

ALTER TABLE public.user_actor_interactions ENABLE ROW LEVEL SECURITY;

CREATE POLICY uai_user_select ON public.user_actor_interactions FOR SELECT
  TO authenticated
  USING (user_id = auth.uid() OR is_admin(auth.uid()));

CREATE POLICY uai_user_insert ON public.user_actor_interactions FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

-- 2) user_preferences (auth.users is locked, separate table per Lovable best practice)
CREATE TABLE public.user_preferences (
  user_id uuid PRIMARY KEY,
  default_axis_weights jsonb,
...
-- 3) Recreate v2 RPC with engagement axis + p_user_id parameter, rebalance defaults.
...
AS $function$
DECLARE
  -- Default weights (AX4 rebalance: cert 0.10 → 0.07, engagement 0.03)
