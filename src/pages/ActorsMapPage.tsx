...
const VIEW_STORAGE_KEY = "actorsMapView";
...
function FitOnFirstLoad({ actors }: { actors: ActorsMapEntry[] }) {
  const map = useMap();
...
const ActorsMapPage = () => {
  ...
  const allDomains = useMemo(() => {
    ...
    for (const a of data) if (a.primary_domain_category) s.add(a.primary_domain_category);
    return Array.from(s).sort();
  }, [data]);

  const allCountries = useMemo(() => {
    const m = new Map<string, string>(); // iso → display label
    for (const a of data) {
    ...
    <div className="h-full flex flex-col bg-background overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-border bg-elevated/40 shrink-0">
        ...
        <div className="mt-2 text-body-sm text-foreground-secondary">
          Actors on map · {totalPlotted} of {data.length} plotted
      ...
      <div className="flex-1 flex min-h-0">
        {/* Filter sidebar */}
        <aside className="w-64 border-r border-border bg-elevated/30 overflow-y-auto shrink-0">
        ...
        <div ref={mapBoxRef} className="flex-1 relative min-w-0">
          ...
          >
            ...
            <ViewPersistence />
            <FitOnFirstLoad actors={geocoded} />
