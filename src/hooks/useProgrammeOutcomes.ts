...
export function useProgrammeOutcomes(programmeId: string | undefined) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [actorsRes, usersRes, progRes] = await Promise.allSettled([
        supabase.from("actors").select("id, legal_name").in("id", actorIds),
        userIds.length
          ? supabase.from("users").select("id, name, email").in("id", userIds)
          : Promise.resolve({ data: [] as { id: string; name: string; email: string }[], error: null }),
        supabase.from("programmes").select("name").eq("id", programmeId).maybeSingle(),
      ]);

      const actors = (actorsRes.status === "fulfilled" && !(actorsRes.value as any).error)
        ? ((actorsRes.value as any).data ?? [])
        : (toast.error(`Failed to load outcome actors`), []);
      ...
      const prog = (progRes.status === "fulfilled" && !(progRes.value as any).error)
        ? (progRes.value as any).data
        : null;

      const actorMap = new Map((actors as any[]).map((a: any) => [a.id, a.legal_name]));
      ...
      toast.error(`Failed to load outcomes: ${e?.message ?? "Unknown error"}`);
      setOutcomes([]);
    } finally {
      setLoading(false);
    }
